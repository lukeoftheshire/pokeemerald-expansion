 ...
 * Task_NewGameBirchSpeech_StartBirchTyranitarPlatformFade
 * Task_NewGameBirchSpeech_StartBirchTyranitarPlatformFade
 ...
 * Task_NewGameBirchSpeech_ReshowBirchTyranitar
 ...
 *  - Advances to Task_NewGameBirchSpeechSub_WaitForTyranitar
 * Task_NewGameBirchSpeechSub_WaitForTyranitar
...
static void Task_NewGameBirchSpeechSub_WaitForTyranitar(u8);
static void Task_NewGameBirchSpeech_StartBirchTyranitarPlatformFade(u8);
...
static void Task_NewGameBirchSpeech_ReshowBirchTyranitar(u8);
...
#define tTyranitarSpriteId data[9]
...
{
    u8 spriteId = gTasks[sBirchSpeechMainTaskId].tTyranitarSpriteId;
    ...
    CreatePokeballSpriteToReleaseMon(spriteId, gSprites[spriteId].oam.paletteNum, 112, 58, 0, 0, 32, PALETTES_BG, SPECIES_TYRANITAR);
    gTasks[taskId].func = Task_NewGameBirchSpeechSub_WaitForTyranitar;
    gTasks[sBirchSpeechMainTaskId].tTimer = 0;
}

static void Task_NewGameBirchSpeechSub_WaitForTyranitar(u8 taskId)
{
    s16 *data = gTasks[taskId].data;
    struct Sprite *sprite = &gSprites[gTasks[sBirchSpeechMainTaskId].tTyranitarSpriteId];
...
static void Task_NewGameBirchSpeech_AndYouAre(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        ...
        AddTextPrinterForMessage(TRUE);
        gTasks[taskId].func = Task_NewGameBirchSpeech_StartBirchTyranitarPlatformFade;
    }
}

static void Task_NewGameBirchSpeech_StartBirchTyranitarPlatformFade(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        gSprites[gTasks[taskId].tTyranitarSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static void Task_NewGameBirchSpeech_StartPlayerFadeIn(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        gSprites[gTasks[taskId].tTyranitarSpriteId].invisible = TRUE;
        if (gTasks[taskId].tTimer)
        {
            gTasks[taskId].tTimer--;
        }
...
static void Task_NewGameBirchSpeech_StartNamingScreen(u8 taskId)
{
    ...
    {
        FreeAllWindowBuffers();
        FreeAndDestroyMonPicSprite(gTasks[taskId].tTyranitarSpriteId);
...
static void Task_NewGameBirchSpeech_SlidePlatformAway2(u8 taskId)
{
    ...
    {
        gTasks[taskId].func = Task_NewGameBirchSpeech_ReshowBirchTyranitar;
    }
}

static void Task_NewGameBirchSpeech_ReshowBirchTyranitar(u8 taskId)
{
    u8 spriteId;
    ...
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        gSprites[spriteId].invisible = FALSE;
        gSprites[spriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        spriteId = gTasks[taskId].tTyranitarSpriteId;
        gSprites[spriteId].x = 100;
        gSprites[spriteId].y = 75;
        gSprites[spriteId].invisible = FALSE;
        gSprites[spriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static void Task_NewGameBirchSpeech_WaitForSpriteFadeInAndTextPrinter(u8 taskId)
{
    ...
    {
        gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        gSprites[gTasks[taskId].tTyranitarSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        if (!RunTextPrintersAndIsPrinter0Active())
        {
            gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
            gSprites[gTasks[taskId].tTyranitarSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static u8 NewGameBirchSpeech_CreateTyranitarSprite(u8 x, u8 y)
{
    return CreateMonPicSprite_Affine(SPECIES_TYRANITAR, FALSE, 0, MON_PIC_AFFINE_FRONT, x, y, 14, TAG_NONE);
}

static void AddBirchSpeechObjects(u8 taskId)
{
    u8 birchSpriteId;
    u8 tyranitarSpriteId;
    u8 brendanSpriteId;
    u8 maySpriteId;
    ...
    tyranitarSpriteId = NewGameBirchSpeech_CreateTyranitarSprite(100, 0x4B);
    gSprites[tyranitarSpriteId].callback = SpriteCB_Null;
    gSprites[tyranitarSpriteId].oam.priority = 0;
    gSprites[tyranitarSpriteId].invisible = TRUE;
    gTasks[taskId].tTyranitarSpriteId = tyranitarSpriteId;
